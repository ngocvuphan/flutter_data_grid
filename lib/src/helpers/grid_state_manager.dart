...
class GridStateManager extends ChangeNotifier {
  ...
  bool isIntrinsicWidthColumn(int index) =>
      columns[index].columnWidth is IntrinsicDataGridColumnWidth;
  ...
  double get gridWidth =>
      math.min(_maxWidth, _columnWidths.fold(0.0, (p, v) => p + v));

  double getColumnWidth(int index) =>
      index < _columnWidths.length ? _columnWidths[index] : _minColumnWidth;
  ...
  void computeColumnWidths({List<Map<int, double>>? intrinsicWidths}) {
    ...
    for (int x = 0; x < columns.length; x++) {
      ...
      if (column.columnWidth is FixedDataGridColumnWidth) {
        allocatedWidth += _columnWidths[x] = _columnWidths[x] > _minColumnWidth
            ? _columnWidths[x]
        ...
        double width = intrinsicWidths == null
            ? math.max(columnWidth.minWidth, _columnWidths[x])
            : intrinsicWidths.map((e) => e[x] ?? 0.0).reduce(math.max);
    ...
    if (totalFlex > 0) {
      final widthPerFlex =
          math.max((_maxWidth - allocatedWidth) / totalFlex, _minColumnWidth);
      for (int x = 0; x < columns.length; x++) {
        final column = columns[x];
        if (column.columnWidth is FlexDataGridColumnWidth) {
          _columnWidths[x] = column.columnWidth.value * widthPerFlex;
        }
      }
    }
    // Fire event
  ...
  void _updateColumnWidths(int column, double delta) {
    if (column < _columnWidths.length) {
      ...
      _columnWidths[column] =
          math.max(_minColumnWidth, _columnWidths[column] + delta);
      // Fill last column
      final lastWidth =
          _maxWidth - _columnWidths.fold(-_columnWidths.last, (p, v) => p + v);
